...
这时就需要开发人员介入了，使用 dependecy 的 exclusions
...
但是，用户不能随意更改这个配置，因为它必须得到一定的授权，否则随意忽略会产生其他不确定的问题。因此我们将这个插件做了一些改动，通过API来获取
...
该逻辑的含义是： 当项目的依赖存在 foo.dll 和 bar.dll 时，bar.dll
